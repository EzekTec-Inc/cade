...
---
**UTC Timestamp:** 2026-05-11T16:00:00Z
**Summary of change:** Extract `StorageBackend` trait and unify CLI and Server meta-tool handlers.
**Files modified:**
- `crates/cade-agent/src/backends/storage.rs` (created)
- `crates/cade-agent/src/agent/client/storage_impl.rs` (created)
- `crates/cade-server/src/server/api/run/storage_impl.rs` (created)
- `crates/cade-agent/src/tools/runtime/mod.rs`
- `crates/cade-server/src/server/api/run/mod.rs`
- `crates/cade-server/src/server/api/run/subagent.rs`
- `crates/cade-server/src/server/api/run/meta_tools.rs` (deleted)
- `crates/cade-server/src/server/api/run/tests.rs`

**Reason:**
As part of the Medium Term roadmap goals, extracting a `StorageBackend` trait unifies the CLI (`HttpTransport`) and the Server (direct SQLite access) meta-tool handlers. This completely eliminates ~1,500 lines of heavily duplicated code inside the server's `meta_tools.rs`.

**Previous behavior:**
`ToolRuntime` in `cade-agent` had a hard dependency on `Arc<HttpTransport>` and executed meta-tools via HTTP calls. The `cade-server` agentic loop bypassed `ToolRuntime` for meta-tools, manually intercepting them with duplicate implementations in `meta_tools.rs` that talked directly to the database.

**New behavior:**
- Defined `StorageBackend` trait for all meta-tool operations (memory, skills, checkpoints, artifacts).
- Implemented `StorageBackend` for `HttpTransport` (CLI) and `ServerStorageBackend` (Server).
- Refactored `ToolRuntime` to accept `Arc<dyn StorageBackend>`.
- Replaced the duplicate `intercept_meta_tool` loop in the server with standard `ToolRuntime` dispatching.
- Removed obsolete and tightly coupled tests.
