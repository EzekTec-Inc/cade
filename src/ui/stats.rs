...
impl SessionStats {
    pub fn new() -> Self {
        Self {
            pub started_at:       std::time::Instant::now(),
            pub agent_active_ms:  0,
            pub api_time_ms:      0,
            pub tool_time_ms:     0,
            pub tool_calls_total: 0,
            pub tool_calls_ok:    0,
            pub tool_calls_err:   0,
            pub approved:         0,
            pub reviewed:         0,
            pub lines_added:      0,
            pub lines_removed:    0,
            pub per_model:        std::collections::HashMap::new(),
        }
    }

    /// Record a usage_statistics SSE event.
    pub fn record_usage(&mut self, model: &str, input: u64, cache_read: u64, cache_write: u64, output: u64) {
    ...
    pub fn compute_cost(&self) -> (f64, Vec<(String, f64)>) {
    ...
    pub fn render_card(&self, auth_method: &str, session_id: &str) -> Vec<crate::ui::RenderLine> {
    ...
    pub fn render_model_detail(&self) -> Vec<crate::ui::RenderLine> {
        use crate::ui::RenderLine;
