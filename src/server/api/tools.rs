...
use crate::server::{state::AppState, storage::sqlite::{self, ToolRow}};
...
pub async fn create_tool(
    ...
    tracing::debug!("Registered tool: {name} ({id})");
    Ok(Json(json!({ "id": id, "name": name, "description": row.description })))
}
...
pub async fn list_tools(
    ...
    let tools: Vec<Value> = rows.iter().map(|t| json!({
        "id": t.id,
        "name": t.name,
