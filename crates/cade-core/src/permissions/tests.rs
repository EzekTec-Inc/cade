...
impl PermissionService for MockPermissionService {
    async fn request_permission(&self, _tool_name: &str, _args: &serde_json::Value) -> std::result::Result<bool, String> {
        ...
        Ok(self.approve.load(std::sync::atomic::Ordering::Relaxed))
    }
}

#[tokio::test]
async fn test_security_authority_delegates_to_service_on_ask() {
    use crate::permissions::authority::SecurityAuthority;
    use crate::permissions::manager::PermissionManager;
    use crate::permissions::rules::Verdict;
    use serde_json::json;
    ...
    let mgr = PermissionManager::new(PermissionMode::Default);
    let args = json!({"path": "normal.rs", "content": "code"});

    // 1. Mock service approves the request
    let service_approve = MockPermissionService {
        approve: std::sync::atomic::AtomicBool::new(true),
        feedback: None,
    };
    ...
    let service_deny = MockPermissionService {
    ...
    let service_feedback = MockPermissionService {
        approve: std::sync::atomic::AtomicBool::new(false),
