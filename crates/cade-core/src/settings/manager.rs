...
mod tests {
    ...
    fn hooks_config_merge_preserves_both() {
        ...
        a.pre_tool_use.push(HookEntry { matcher: None, hooks: vec![] });
        let mut b = HooksConfig::default();
        b.stop.push(HookEntry { matcher: None, hooks: vec![] });
    ...
    #[test]
    fn global_settings_store_api_key_defaults_true() {
        let json = r#"{}"#;
    ...
    fn project_settings_with_mcp_servers() {
        ...
        let ps: ProjectSettings = serde_json::from_str(json).unwrap();
    ...
    fn local_settings_pinned_agents() {
        ...
        let ls: LocalSettings = serde_json::from_str(json).unwrap();
    ...
    fn hook_def_json_roundtrip() {
        let hook = HookDef::Command { command: "echo hello".into(), timeout: 30000 };
    ...
    fn settings_manager_loads_defaults_for_missing_files() {
        ...
        let mgr = SettingsManager::new(dir.path()).unwrap();
    ...
    #[test]
    fn settings_manager_merged_mcp_servers() {
        let dir = tempfile::tempdir().unwrap();
        let cade_dir = dir.path().join(".cade");
        fs::create_dir_all(&cade_dir).unwrap();
        ...
        fs::write(cade_dir.join("settings.json"), project_json).unwrap();
        ...
        fs::write(cade_dir.join("settings.local.json"), local_json).unwrap();

        let mgr = SettingsManager::new(dir.path()).unwrap();
        let servers = mgr.merged_mcp_servers();

        // Local override wins for proj-server
        assert_eq!(servers.get("proj-server").unwrap().command, "/bin/local-proj");
        // Local-only server is present
        assert!(servers.contains_key("local-only"));
    }

    #[test]
    fn settings_manager_disabled_mcp_server_excluded() {
        let dir = tempfile::tempdir().unwrap();
        let cade_dir = dir.path().join(".cade");
        fs::create_dir_all(&cade_dir).unwrap();
    ...
    #[test]
    fn settings_manager_set_and_get_last_agent() {
        let dir = tempfile::tempdir().unwrap();
        let cade_dir = dir.path().join(".cade");
        fs::create_dir_all(&cade_dir).unwrap();
        ...
        let mut mgr = SettingsManager::new(dir.path()).unwrap();
    ...
    #[test]
    fn settings_manager_pin_agent() {
        let dir = tempfile::tempdir().unwrap();
        let cade_dir = dir.path().join(".cade");
        fs::create_dir_all(&cade_dir).unwrap();

        let mut mgr = SettingsManager::new(dir.path()).unwrap();
        mgr.pin_agent("a1", "Agent One").unwrap();
        mgr.pin_agent("a2", "Agent Two").unwrap();
        assert_eq!(mgr.pinned_agents().len(), 2);

        // Pin same ID again — deduplicates
        mgr.pin_agent("a1", "Agent One Updated").unwrap();
        assert_eq!(mgr.pinned_agents().len(), 2);
        assert_eq!(mgr.pinned_agents().iter().find(|p| p.id == "a1").unwrap().name, "Agent One Updated");
    }

    #[test]
    fn settings_manager_save_and_load_rules() {
        let dir = tempfile::tempdir().unwrap();
        let cade_dir = dir.path().join(".cade");
        fs::create_dir_all(&cade_dir).unwrap();
        ...
        assert!(mgr.permission_settings().allow.contains(&"Bash(cargo test)".to_string()));
        assert!(mgr.permission_settings().deny.contains(&"Bash(rm:*)".to_string()));
    }

    #[test]
    fn settings_manager_base_url_default() {
        let dir = tempfile::tempdir().unwrap();
        let mgr = SettingsManager::new(dir.path()).unwrap();
    ...
    #[test]
    fn settings_manager_merged_hooks() {
        let dir = tempfile::tempdir().unwrap();
        let cade_dir = dir.path().join(".cade");
        fs::create_dir_all(&cade_dir).unwrap();
