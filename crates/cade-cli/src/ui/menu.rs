...
const SECTIONS: &[Section] = &[
    ...
    Section { name: "Tools & Providers", items: &[
    ...
    Section { name: "Skills & Subagents", items: &[
        CmdEntry { cmd: "/skills",        desc: "List loaded skills" },
        CmdEntry { cmd: "/skills create", desc: "/skills create <name>  — scaffold a new skill" },
...
pub fn show_command_menu(terminal: &mut DefaultTerminal) -> Result<Option<String>> {
    ...
    loop {
        let list_items: Vec<ListItem<'static>> = items.iter().enumerate().map(|(i, item)| {
            match item {
                ...
                MenuItem::Cmd { cmd, desc } => {
                    ...
                    ListItem::new(Line::from(vec![
                        ...
                        Span::styled(format!("{cmd:<22}"), Style::default()
                            .fg(if is_sel { Color::White } else { Color::Rgb(160, 160, 230) })
        ...
        match event::read()? {
            Event::Key(k) => match k.code {
                ...
                KeyCode::Up   | KeyCode::Char('k') => { sel = prev_cmd(&items, sel); }
                KeyCode::Down | KeyCode::Char('j') => { sel = next_cmd(&items, sel); }
                KeyCode::PageUp   => { for _ in 0..8 { sel = prev_cmd(&items, sel); } }
                KeyCode::PageDown => { for _ in 0..8 { sel = next_cmd(&items, sel); } }
                _ => {}
            },
            _ => {}
        }
