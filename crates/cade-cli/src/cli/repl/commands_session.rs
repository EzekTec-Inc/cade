...
impl Repl {
    ...
    pub(crate) async fn cmd_mouse(&mut self) -> Result<bool> {
        let mut app = self.app.lock();
    ...
    pub(crate) async fn cmd_copy(&mut self) -> Result<bool> {
        match self
            .client
            .last_assistant_message(&self.agent_id(), self.conversation_id.lock().as_deref())
            .await
        {
            Ok(Some(msg)) => {
                ...
                if text.is_empty() {
                    self.app.lock().show_toast(
                        "No text found in last assistant message",
                        ToastLevel::Warning,
                    );
                    return Ok(false);
                }
                ...
                self.app
                    .lock()
                    .show_toast("Copied last message to clipboard", ToastLevel::Success);
            }
            Ok(None) => {
                self.app
                    .lock()
                    .show_toast("No assistant message found to copy", ToastLevel::Warning);
            }
            Err(e) => {
                self.app.lock().show_toast(
                    format!("Failed to fetch last message: {e}"),
                    ToastLevel::Error,
                );
            }
        }
