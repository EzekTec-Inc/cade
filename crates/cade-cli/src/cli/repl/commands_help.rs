...
impl Repl {
    ...
    pub(crate) async fn cmd_info(&mut self) -> Result<bool> {
        let msg = format!(
            "  Agent   : {} ({})\n  Conv    : {}\n  Model   : {}\n  Mode    : {}\n  CWD     : {}\n  Version : {}",
            self.agent_name(),
            self.agent_id(),
    ...
    pub(crate) async fn cmd_usage(&mut self, _pending_input: &mut Option<String>) -> Result<bool> {
    ...
    pub(crate) async fn cmd_stats(&mut self, arg: Option<String>) -> Result<bool> {
        let sub = arg.as_deref().unwrap_or("").trim();
        let lines = match sub {
            "model" | "models" => self.session_stats.lock().render_model_detail(),
            _ => {
    ...
    pub(crate) async fn cmd_debug_last(
            ...
            .await
        {
            ...
            Err(e) => self.tui_err(format!("Failed to load last assistant message: {e}")),
        }
        Ok(false)
    }
}
