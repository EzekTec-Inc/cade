...
impl Repl {
    pub(crate) async fn cmd_marketplace(&self) -> Result<bool> {
        ...
        let result = self.marketplace_picker(self.app.clone(), &index.plugins).await?;
        
        if let Some(crate::cli::repl::pickers::marketplace::MarketplaceActionResult::Install(url, plugin_id)) = result {
            self.tui_sys(format!("Installing plugin {plugin_id}..."));
            let target_dir = dirs::home_dir().map(|h| h.join(".cade").join("plugins")).unwrap_or_default();
            match cade_plugin::marketplace::install_plugin(&url, &plugin_id, &target_dir).await {
                Ok(manifest) => {
                    self.tui_ok(format!("Successfully installed plugin '{}' (v{})", manifest.name, manifest.version.unwrap_or_else(|| "unknown".to_string())));
                }
                Err(e) => {
