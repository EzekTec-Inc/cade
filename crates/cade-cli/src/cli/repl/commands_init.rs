...
impl Repl {
    pub(crate) async fn cmd_init(&mut self, stdout: &mut std::io::Stdout) -> Result<bool> {
        ...
        let summary = {
            ...
            let _system_prompt = explore_def.map(|d| d.system_prompt).unwrap_or_else(|| {
                "You are an expert code explorer. Be concise and precise.".to_string()
            });
