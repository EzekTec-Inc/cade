...
impl Repl {
    ...
    pub(crate) async fn cmd_memory(
        ...
        match sub {
            ...
            "edit" if parts.len() >= 2 => {
                ...
                let opts = vec![
                    QuestionOption {
                        label: format!("Keep: {}…", current.chars().take(60).collect::<String>()),
                    ...
                    Ok(revs) => {
                        ...
                        for (i, rev) in revs.iter().enumerate() {
                            ...
                            let _ = self
                                .app
                                .lock()
                                .push(RenderLine::SystemMsg(format!("  [{i}] {ts}  id={rev_id}")));
                            let _ = self
                                .app
                                .lock()
                                .push(RenderLine::SystemMsg(format!("      {preview}{ellipsis}")));
                            let _ = self.app.lock().push(RenderLine::Blank);
                        }
                        let _ = self.app.lock().push(RenderLine::SystemMsg(format!(
            ...
            "export" => {
                ...
                match self.client.export_memory(&id, path_arg.as_deref()).await {
            ...
            "audit" => {
                ...
                match self.client.get_memory(&id).await {
                    Ok(blocks) => {
                        ...
                        self.tui_dim(
                            "  Use /reflect to trigger automatic extraction from conversation."
                                .to_string(),
                        );
                    }
                    Err(e) => self.tui_err(e.to_string()),
            ...
            _ => {
                ...
                match self.client.get_memory(&id).await {
                    Ok(mut blocks) => {
                        match self
                            .memory_picker(std::sync::Arc::clone(&self.app), &mut blocks)
                            .await
                        {
    ...
    pub(crate) async fn cmd_reflect(&mut self, focus_arg: Option<String>) -> Result<bool> {
    ...
    pub(crate) async fn cmd_remember(&mut self, text: String) -> Result<bool> {
    ...
    pub(crate) async fn cmd_pin(&mut self) -> Result<bool> {
        let id = self.agent_id();
        let name = self.agent_name();
        {
            ...
            match s.pin_agent(&id, &name) {
                Ok(_) => {
                    self.app
                        .lock()
                        .show_toast(format!("Pinned agent: {name}"), ToastLevel::Success);
