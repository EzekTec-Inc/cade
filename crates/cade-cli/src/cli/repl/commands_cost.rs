...
impl Repl {
    pub(crate) async fn cmd_cost(&mut self) -> Result<bool> {
            ...
            for (model, cost) in &by_model {
                    ...
                    .map(|(_, v)| v)
                {
                    ...
                    if ms.output_tokens > 0 {
                        fields.push(format!("{} output", fmt_tok(ms.output_tokens)));
                    }
                    if ms.cache_read_tokens > 0 {
                        fields.push(format!("{} cache read", fmt_tok(ms.cache_read_tokens)));
                    }
                    if ms.cache_write_tokens > 0 {
                        fields.push(format!("{} cache write", fmt_tok(ms.cache_write_tokens)));
                    }
                    if !fields.is_empty() {
                        lines.push(crate::ui::RenderLine::DimMsg(format!(
                            "       {}",
