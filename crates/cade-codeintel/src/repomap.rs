...
pub fn generate_repo_map(repo_root: &Path, db: &Db, max_symbols_per_file: usize) -> Result<String> {
    ...
    let mut by_file: HashMap<String, Vec<(String, String, u32, Option<String>, Option<String>)>> = HashMap::new();
    ...
    for row in rows.filter_map(|r| r.ok()) {
        ...
        by_file.entry(file_path).or_default().push((name, kind, line, parent, sig));
    }
    drop(stmt);
    ...
    for file_path in &files_ordered {
        ...
        for (name, kind, line, parent, sig) in entries.iter().take(limit) {
            let parent_str = parent.as_deref().map(|p| format!("{p}::")).unwrap_or_default();
            let sig_str = sig.as_deref().unwrap_or(name.as_str());
            out.push_str(&format!("  {kind} {parent_str}{name}  (line {line})  {sig_str}\n"));
        }
