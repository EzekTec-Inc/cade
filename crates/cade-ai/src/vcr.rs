...
impl VcrCassette {
    pub fn new(path: PathBuf, mode: VcrMode) -> Result<Self> {
        let interactions = if mode == VcrMode::Replay {
            let mut file = File::open(&path).map_err(|e| {
                crate::Error::custom(format!("Failed to open cassette file: {:?}", e))
            })?;
            let mut content = String::new();
            file.read_to_string(&mut content).map_err(|e| {
                crate::Error::custom(format!("Failed to read cassette file: {:?}", e))
            })?;
            serde_json::from_str(&content).map_err(|e| {
                crate::Error::custom(format!(
                    "Failed to deserialize cassette interactions: {:?}",
                    e
                ))
            })?
        } else {
    ...
    pub fn record_interaction(&self, mut interaction: HttpInteraction) -> Result<()> {
        ...
        interaction.method = redact_secrets(&interaction.method);
        ...
        let content = serde_json::to_string_pretty(&*lock).map_err(|e| {
            crate::Error::custom(format!("Failed to serialize interactions: {:?}", e))
        })?;
        let mut file = OpenOptions::new()
            ...
            .open(&self.path)
            .map_err(|e| {
                crate::Error::custom(format!("Failed to open cassette for writing: {:?}", e))
            })?;
