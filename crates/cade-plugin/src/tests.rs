...
"#;
        ...
        assert_eq!(manifest.description.as_deref(), Some("A test plugin in TOML"));
    ...
    fn test_load_json_manifest() {
        ...
        std::fs::write(tmp.path().join("cade-plugin.json"), json_content.to_string()).unwrap();
    ...
    fn test_plugin_init_and_validate() {
        ...
        assert!(report.is_valid, "scaffolded plugin must be valid: {:?}", report.issues);
