    ...
    rsx! {
        div { class: "flex-1 bg-[#040711] h-full overflow-y-auto select-text",
            header { class: "px-10 py-4 flex items-center justify-between select-none border-b border-[#1e293b]/70",
                h1 { class: "text-lg font-semibold text-slate-100", "Models" }
            }
            div { class: "p-10 space-y-6",
                div { class: "space-y-3",
                    h2 { class: "text-sm font-semibold text-slate-100", "Available Models" }
                    if fetching() {
                        ...
                        div { class: "bg-[#090d16] border border-[#1e293b] rounded-xl p-8 text-center",
                            p { class: "text-slate-500 text-sm", "No models available. Configure a provider first." }
                        }
                    } else {
                        {supported.into_iter().map(|m| {
                            ...
                            rsx! {
                                div { class: "bg-[#090d16] border border-[#1e293b] rounded-xl px-5 py-4 flex items-center justify-between",
                                    div { class: "flex flex-col",
                                        span { class: "text-slate-100 text-sm font-medium", "{id}" }
                                        if !provider.is_empty() {
                                            span { class: "text-slate-500 text-xs", "{provider}" }
                                        }
                                    }
                                    if context > 0 {
                                        span { class: "text-slate-400 text-xs", "{context / 1000}K context" }
                                    }
                                }
                            }
                        })}
                        if !dynamic.is_empty() {
                            div { class: "text-[10px] font-bold text-slate-500 tracking-wider uppercase pt-2", "Dynamic / Available" }
                            {dynamic.into_iter().map(|m| {
                                let id = m.get("id").and_then(|v| v.as_str()).unwrap_or("?").to_string();
                                let provider = m.get("provider").and_then(|v| v.as_str()).unwrap_or("").to_string();
                                rsx! {
                                    div { class: "bg-[#090d16]/40 border border-[#1e293b]/60 rounded-xl px-5 py-3 flex items-center justify-between",
                                        div { class: "flex flex-col",
                                            span { class: "text-slate-300 text-sm", "{id}" }
                                            if !provider.is_empty() {
                                                span { class: "text-slate-500 text-xs", "{provider}" }
                                            }
                                        }
