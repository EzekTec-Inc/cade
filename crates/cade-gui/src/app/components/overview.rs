    ...
    egui::ScrollArea::vertical().show(ui, |ui| {
        ...
        ui.columns(2, |columns| {
            ...
            columns[0].vertical(|ui| {
                // What's New Section
                egui::Frame::none().inner_margin(8.0).fill(theme.bg_surface0()).rounding(8.0).show(ui, |ui| {
                ...
                egui::Frame::none().inner_margin(8.0).fill(theme.bg_surface0()).rounding(8.0).show(ui, |ui| {
                    ...
                    egui::Grid::new("config_grid").num_columns(2).spacing([40.0, 8.0]).show(ui, |ui| {
                        ui.label(egui::RichText::new("Model:").color(theme.text_secondary()));
                        let model = session.context_stats.as_ref().and_then(|c| c.model.clone()).unwrap_or_else(|| "Auto-detect".into());
                        ui.label(model);
                        ui.end_row();
                        
                        ui.label(egui::RichText::new("Available Tools:").color(theme.text_secondary()));
                        ui.label(session.tools.len().to_string());
                        ui.end_row();
                        
                        ui.label(egui::RichText::new("MCP Servers:").color(theme.text_secondary()));
                ...
                egui::Frame::none().inner_margin(8.0).fill(theme.bg_surface0()).rounding(8.0).show(ui, |ui| {
                    ...
                    egui::Grid::new("usage_grid").num_columns(2).spacing([40.0, 8.0]).show(ui, |ui| {
                        ui.label(egui::RichText::new("Total Input Tokens:").color(theme.text_secondary()));
                        ui.label(total_input.to_string());
                        ui.end_row();
                        
                        ui.label(egui::RichText::new("Total Output Tokens:").color(theme.text_secondary()));
                        ui.label(total_output.to_string());
                        ui.end_row();
                        
                        ui.label(egui::RichText::new("Combined Total:").color(theme.text_secondary()));
            ...
            columns[1].vertical(|ui| {
                ...
                egui::Frame::none().inner_margin(8.0).fill(theme.bg_surface0()).rounding(8.0).show(ui, |ui| {
                    ...
                    egui::Grid::new("metrics_grid").num_columns(2).spacing([40.0, 8.0]).show(ui, |ui| {
                        ui.label(egui::RichText::new("Compacted Data:").color(theme.text_secondary()));
                        ui.label(compacted.to_string());
                        ui.end_row();
                        
                        ui.label(egui::RichText::new("Consolidations:").color(theme.text_secondary()));
                        ui.label(runs.to_string());
                        ui.end_row();
                        
                        ui.label(egui::RichText::new("Guard Hits:").color(theme.text_secondary()));
                ...
                egui::Frame::none().inner_margin(8.0).fill(theme.bg_surface0()).rounding(8.0).show(ui, |ui| {
                    ...
                    egui::Grid::new("last_exec_grid").num_columns(2).spacing([40.0, 8.0]).show(ui, |ui| {
                        ...
                        ui.label(egui::RichText::new("Turn Tokens:").color(theme.text_secondary()));
                ...
                egui::Frame::none().inner_margin(8.0).fill(theme.bg_surface0()).rounding(8.0).show(ui, |ui| {
                    ...
                    if cards.is_empty() {
                        ui.label(egui::RichText::new("No active background executions.").color(theme.text_secondary()).italics());
                    } else {
                        for card in cards {
                            ui.horizontal(|ui| {
                                ui.label(egui::RichText::new("•").color(theme.primary()));
                                ui.label(format!("{} - ", card.task));
                                ui.label(egui::RichText::new(&card.status).color(theme.text_secondary()));
                            });
                        }
                    }
                });
