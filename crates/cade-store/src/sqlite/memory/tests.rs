...
fn test_a15_write_back_avoids_overwriting_parent() -> Result<()> {
    ...
    let parent_blocks = get_memory_blocks(&db, "parent")?;
    let orig_finding = parent_blocks
        .iter()
        .find(|(l, _, _)| l == "subagent:finding")
        .map(|(_, v, _)| v.as_str());
    let fallback_finding = parent_blocks
        .iter()
        .find(|(l, _, _)| l == "subagent:finding:fallback")
        .map(|(_, v, _)| v.as_str());

    assert_eq!(
        orig_finding,
        Some("parent original value"),
        "original parent fact must be preserved"
    );
    assert_eq!(
        fallback_finding,
        Some("subagent finding value"),
        "subagent fallback fact must be created"
    );
