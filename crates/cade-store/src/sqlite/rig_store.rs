...
impl rig::vector_store::VectorStoreIndex for RigCadeStore {
    fn top_n<T: for<'a> serde::Deserialize<'a> + Send>(
        ...
        async move {
            ...
            for r in rows {
                let (id, text) = r.map_err(|e| VectorStoreError::DatastoreError(Box::from(e.to_string())))?;
    ...
    fn top_n_ids(
        ...
        async move {
            ...
            let limit = n as i64;
            let mut stmt = conn.prepare(
                "SELECT id FROM memory_embeddings LIMIT ?"
            ).map_err(|e| VectorStoreError::DatastoreError(Box::from(e.to_string())))?;
            ...
            let mut results = Vec::new();
            for r in rows {
                results.push(r.map_err(|e| VectorStoreError::DatastoreError(Box::from(e.to_string())))?);
            }
            Ok(results)
        }
    }
