...
pub async fn create_git_checkpoint(label: &str, cwd: &Path) -> Option<GitCheckpoint> {
    if !is_git_repo(cwd).await { return None; }
    ...
    Some(GitCheckpoint { stash_ref, commit_hash })
}
...
pub async fn restore_git_checkpoint(stash_ref: &str, cwd: &Path) -> Result<()> {
    if stash_ref.is_empty() { return Ok(()); }
    let out = run_git(cwd, &["stash", "apply", stash_ref]).await;
    match out {
        Some((exit, _, stderr)) if exit != 0 => {
            Err(crate::Error::custom(format!("git stash apply failed: {stderr}")))
        }
...
pub async fn current_git_hash(cwd: &Path) -> Option<String> {
    ...
    if exit == 0 { Some(stdout.trim().to_string()) } else { None }
}
...
async fn is_git_repo(cwd: &Path) -> bool {
    matches!(run_git(cwd, &["rev-parse", "--git-dir"]).await, Some((0, _, _)))
}
