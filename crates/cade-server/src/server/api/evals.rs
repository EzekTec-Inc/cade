...
pub async fn create_eval_task(
    ...
    let expected = body.expected_json
    ...
    Ok(Json(json!({ "id": id, "name": body.name, "created_at": now })))
}
...
pub async fn create_eval_run_handler(
    ...
    Ok(Json(json!({ "id": id, "status": "pending", "created_at": now })))
}
...
pub async fn update_eval_run_handler(
    ...
    let query = format!("UPDATE eval_runs SET {} WHERE id = ?{param_idx}", sets.join(", "));
    ...
    let n = conn.execute(&query, rusqlite::params_from_iter(sql_params))
        .map_err(db_err)?;
