...
impl StorageBackend for ServerStorageBackend {
    ...
    async fn conversation_search(
        ...
        let results = tokio::task::spawn_blocking({
            ...
            move || cade_store::sqlite::search_messages(&db, &aid, &q, None)
        })
        .await
        .map_err(|e| cade_agent::Error::custom(e.to_string()))?
        .map_err(|e| cade_agent::Error::custom(e.to_string()))?;
    ...
    async fn record_recent_edit(&self, agent_id: &str, path: &str) -> Result<()> {
        ...
        let blocks =
            cade_store::sqlite::get_memory_blocks(&self.state.db, agent_id).unwrap_or_default();
    ...
    async fn trigger_reflect(&self, agent_id: &str, focus: Option<&str>) -> Result<()> {
        ...
        tokio::spawn(async move {
            let _ = crate::server::reflection::reflect_agent(
                &state_c,
                &aid,
                f.as_deref(),
                None,
