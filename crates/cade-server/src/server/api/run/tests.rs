...
use super::meta_tools::{handle_conversation_search_meta, intercept_meta_tool};
...
async fn intercept_meta_tool_dispatches_memory_apply_patch() {
    ...
    cade_store::sqlite::upsert_memory_block(&state.db, "agent_p", "notes", "old line", None, None)
        .unwrap();
...
async fn subagent_loop_dispatches_update_memory_via_intercept() {
    ...
    let result = handle_run_subagent_tool(&state, "parent_sa", "tc_sa", &args, tx).await;
    ...
    let parent_blocks = cade_store::sqlite::get_memory_blocks(&state.db, "parent_sa").unwrap();
...
async fn subagent_loop_dispatches_search_memory_via_intercept() {
    ...
    let result = handle_run_subagent_tool(&state, "parent_sm", "tc_sm", &args, tx).await;
