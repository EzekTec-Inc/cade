...
| `set_plan` | Declare structured task checklists rendered in interactive plan panel |
| `UpdatePlan` | Update step completion status with live real-time checklist feedback |
...
| **Headroom Proxy** | Transparent prompt compression, caching, and token savings at `http://127.0.0.1:8787` (ADR-0023) |
| **Plugins & Marketplace** | Bundle skills, subagents, MCP servers, and reactive Lua UI extensions into verified `.tar.gz` packages ([Plugin Guide](docs/plugin-development.md)) |
...
- **Embedded In-Process SDK (`cade-sdk`)** — Zero-daemon agent execution, declarative multi-agent squads (`TeamSession`), and strongly-typed reactive streams (`CadeStreamEvent`) with official Python and TypeScript bindings. [Learn more →](crates/cade-sdk/README.md)
- **Native Needle 2 Integration** — Ultra-compact 45M Simple Attention Network running in <30MB RAM for zero-network grammar-constrained tool calling, structured extraction, and neural top-k ITS catalogue pruning.
