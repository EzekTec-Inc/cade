...
- **OpenAI MCP Tool Schema Sanitization:** Hardened the OpenAI provider adapter so MCP function schemas always present a top-level object schema, strip unsupported top-level combinators, preserve parameter names that collide with schema metadata keys (for example `properties.title`), normalize OpenAPI-style `nullable: true`, and prune dangling `required` entries before sending tools to OpenAI models.
- **OpenAI Tool Cap Priority for Meta Tools:** Updated OpenAI's 128-tool cap selection to preserve tools by classification tags (`meta`, `core`, `core_mcp`) so completion-critical tools such as `finish_task` remain available when using GPT-5-family models with large MCP toolsets.
