...
-- ── CADE Inline Completions & Interactive Edits ──────────────────────────────
...
if cfg.keymaps ~= false then
  ...
  -- Insert-mode bindings for completions
  if cfg.completions and cfg.completions.enabled ~= false then
  ...
  -- Normal and Visual-mode interactive edit (<leader>ce)
  if (cfg.edit == nil or cfg.edit.enabled ~= false) and km.edit then
    vim.keymap.set({ "n", "v" }, km.edit, cade.hover_edit, { desc = "CADE: interactive edit" })
  end
end
